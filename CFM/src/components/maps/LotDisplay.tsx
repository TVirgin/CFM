...
export const LotDisplay: React.FC<LotDisplayProps> = ({
  ...
  const positionsPerLot = layout.positionsPerLot;
  ...
  const occupiedPositionMap = React.useMemo(() => {
    ...
    recordsInLot.forEach(record => {
      if (record.pos !== undefined) {
        const key = `${record.pos}`; // Key is just the position number now
        if (!map.has(key)) {
          map.set(key, []);
        }
    ...
    return (
      <div key={posNum} className="border rounded-lg p-2 bg-white shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-1">
          {Array.from({ length: layout.plotsPerPosition }, (_, i) => i + 1).map(plotNum => {
            const record = plotMap.get(plotNum);
            const plotIdentifier: PlotIdentifier = {
              block: layout.id, lot: lotNum, pos: posNum, plot: plotNum,
              rawId: record?.id || `empty-${layout.id}-${lotNum}-${posNum}-${plotNum}`
            };
            const isSelected = selectedPlot?.block === layout.id && selectedPlot?.lot === lotNum && selectedPlot?.pos === posNum && selectedPlot?.plot === plotNum;
